    ...
    <b-card v-if="order">
      ...
      <b-row>
        <b-col md="6">
          ...
          <p><strong>Status:</strong> <b-badge variant="info">{{ order.status }}</b-badge></p>
        ...
        <b-col md="6">
          ...
          <ul>
            <li v-for="(s, idx) in order.services" :key="idx">
              {{ s.name }} — {{ s.value.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }) }}
            </li>
          </ul>
          <p class="mt-2">
            <strong>Total:</strong>
            {{ total(order).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }) }}
          </p>
        </b-col>
      </b-row>
...
export default {
  ...
  methods: {
    async loadOrder() {
      const doc = await db.collection('orders').doc(this.orderId).get()
      if (!doc.exists) return
      this.order = { id: doc.id, ...doc.data() }

      const clientDoc = await db.collection('users').doc(this.order.client_id).get()
      this.clientName = clientDoc.exists ? clientDoc.data().name : 'Cliente não encontrado'

      if (this.order.vehicle_id) {
        const vdoc = await db.collection('vehicles').doc(this.order.vehicle_id).get()
        this.vehicleName = vdoc.exists ? `${vdoc.data().model} (${vdoc.data().plate})` : 'Sem veículo'
      } else {
    ...
    async generatePDF() {
      ...
      doc.text(`Status: ${this.order.status}`, 20, (y += 6))
